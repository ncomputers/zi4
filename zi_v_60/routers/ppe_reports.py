...
from zi_v_60.modules.utils import send_email
from zi_v_60.core.config import ANOMALY_ITEMS

router = APIRouter()

def init_context(config: dict, trackers: Dict[int, "FlowTracker"], redis_client, templates_path, base_dir):
    global cfg, trackers_map, redis, templates, BASE_DIR
    cfg = config
...
async def ppe_report_data(start: str, end: str, status: str = '', min_conf: float | None = None, color: str | None = None):
    ...
    entries = redis.lrange('ppe_logs', 0, -1)
    rows = []
    ...
    for item in entries:
        ...
        rows.append({'time': datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M'), 'cam_id': e.get('cam_id'), 'track_id': e.get('track_id'), 'status': e.get('status'), 'conf': e.get('conf'), 'color': e.get('color'), 'image': img_url})
    return {'rows': rows}
