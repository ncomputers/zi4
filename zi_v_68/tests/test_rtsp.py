...
from zi_v_68.modules.tracker import FlowTracker
...
def make_tracker(src_type):
    with mock.patch('zi_v_68.modules.tracker.YOLO', DummyYOLO), \
         mock.patch('zi_v_68.modules.tracker.redis.Redis.from_url') as rmock:
...
def test_open_capture_local():
    if cv2 is None:
        return
    tr = make_tracker('local')
    tr.src = '1'
    with mock.patch('cv2.VideoCapture') as cap:
        tr._open_capture()
        cap.assert_called_with(1)
