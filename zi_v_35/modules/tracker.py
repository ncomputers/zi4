            ...
            for tid in gone_ids:
                ...
                if img is not None and img.size:
                    ...
                    for *pxyxy, pconf, pcls in pres.boxes.data.tolist():
                        ...
                        if pconf >= self.conf_thresh and plabel in self.alert_anomalies and pconf > best_conf:
                            best_conf = pconf
                            anomaly = plabel
                    logger.info(f"PPE check ID{tid}: {anomaly or 'clear'} conf={best_conf:.2f}")
                if anomaly:
                    snap = img if img is not None else frame
                    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
                    fname = f'{self.cam_id}_{tid}_{anomaly}_{int(best_conf*100)}_{ts}.jpg'
                    ...
                    self.redis.rpush('alerts', json.dumps(entry))
                    to_emails = self.email_cfg.get('ppe_to', '')
                    recipients = [a.strip() for a in to_emails.split(',') if a.strip()]
                    threading.Thread(
                        target=send_email,
                        args=(f'PPE Alert: {anomaly}', f'Camera {self.cam_id} detected {anomaly}', recipients, open(path, 'rb').read(), self.email_cfg),
                        daemon=True,
                    ).start()
