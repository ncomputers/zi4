#!/usr/bin/env python3
"""Crowd management system version 38."""
...
@app.get('/helmet_report')
async def helmet_report_page(request: Request):
    return templates.TemplateResponse('helmet_report.html', {'request': request, 'cfg': config})


@app.get('/helmet_report_data')
async def helmet_report_data(start: str, end: str, status: str = 'no_helmet', min_conf: float | None = None):
    try:
        start_ts = int(datetime.fromisoformat(start).timestamp())
        end_ts = int(datetime.fromisoformat(end).timestamp())
    except Exception:
        return {"error": "invalid range"}
    entries = redis_client.lrange('helmet_logs', 0, -1)
    rows = []
...
@app.get('/helmet_report/export')
async def helmet_report_export(start: str, end: str, status: str = 'no_helmet', min_conf: float | None = None):
    data = await helmet_report_data(start, end, status, min_conf)
    if 'error' in data:
        return JSONResponse(data, status_code=400)
    ...
    wb = Workbook()
    ws = wb.active
    ws.append(['Time', 'Camera', 'Track', 'Status', 'Conf', 'Image'])
    for row in data['rows']:
        ws.append([row['time'], row['cam_id'], row['track_id'], row['status'], round(row['conf'],2)])
    ...
    return StreamingResponse(
        ...
        headers={'Content-Disposition': 'attachment; filename=helmet_report.xlsx'},
    )
