#!/usr/bin/env python3
"""Crowd management system version 53."""
...
CAMERA_TASKS = ["in", "out", "both"] + MODEL_CLASSES
config_path: str
...
def handle_status_change(status: str):
    ...
    ts = int(time.time())
    if status == 'yellow':
        redis_client.incr('yellow_alert_count')
        entry = {
            'ts': ts,
            'cam_id': 0,
            'track_id': 0,
        ...
        redis_client.rpush('ppe_logs', json.dumps(entry))
    elif status == 'red':
        redis_client.incr('red_alert_count')
        entry = {
            'ts': ts,
            'cam_id': 0,
            'track_id': 0,
...
async def index(request: Request):
    ...
    anomaly_counts = {item: int(redis_client.get(f'{item}_count') or 0) for item in ANOMALY_ITEMS}
...
async def update_settings(request: Request):
    ...
    if 'preview_anomalies' in data and isinstance(data['preview_anomalies'], list):
        config['preview_anomalies'] = data['preview_anomalies']
...
async def ppe_report_page(request: Request, status: str = ''):
    statuses = []
    for item in config.get('track_ppe', []):
        statuses.append(item)
        if f'no_{item}' in ANOMALY_ITEMS:
            statuses.append(f'no_{item}')
    if config.get('track_misc', True):
...
async def ppe_report_email(start: str, end: str, status: str = '', min_conf: float | None = None, color: str | None = None, to: str | None = None):
    ...
    wb.save(bio)
    bio.seek(0)
    recipients = [a.strip() for a in (to or config.get('email', {}).get('from_addr', '')).split(',') if a.strip()]
    send_email(
        'PPE Report',
        'See attached report',
        recipients,
...
@app.get('/latest_images')
async def latest_images(status: str = 'no_helmet', count: int = 5):
    entries = redis_client.lrange('ppe_logs', -1000, -1)
    imgs = []
