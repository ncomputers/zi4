#!/usr/bin/env python3
"""Crowd management system version 37."""
...
async def helmet_report_page(request: Request):
    return templates.TemplateResponse('helmet_report.html', {'request': request})


@app.get('/helmet_report_data')
async def helmet_report_data(start: str, end: str, status: str = 'no_helmet'):
    try:
        start_ts = int(datetime.fromisoformat(start).timestamp())
        end_ts = int(datetime.fromisoformat(end).timestamp())
    except Exception:
...
async def helmet_report_export(start: str, end: str, status: str = 'no_helmet'):
    data = await helmet_report_data(start, end, status)
    if 'error' in data:
        return JSONResponse(data, status_code=400)
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Time', 'Camera', 'Track', 'Status', 'Conf'])
    for row in data['rows']:
        writer.writerow([row['time'], row['cam_id'], row['track_id'], row['status'], f"{row['conf']:.2f}"])
    return StreamingResponse(
        io.BytesIO(output.getvalue().encode()),
        media_type='text/csv',
        headers={'Content-Disposition': 'attachment; filename=helmet_report.csv'},
    )
