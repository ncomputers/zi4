#!/usr/bin/env python3
"""Crowd management system version 15."""
...
def load_cameras(r: redis.Redis, default_url: str, default_classes: list[str]) -> list:
    data = r.get("cameras")
    if data:
        try:
...
def start_tracker(cam: dict) -> FlowTracker:
    ...
    tr = FlowTracker(cam["id"], cam["url"], cam.get("classes", config.get("default_classes", ["person"])), config)
    trackers[cam["id"]] = tr
...
async def cameras_page(request: Request):
    return templates.TemplateResponse(
        'cameras.html',
        {'request': request, 'cams': cameras, 'classes': AVAILABLE_CLASSES}
    )
...
async def add_camera(request: Request):
    ...
    classes = data.get('classes') or config.get('default_classes', ['person'])
...
def main():
    ...
    if args.stream_url:
        cameras = [{
            ...
            'mode': 'both',
            'enabled': True,
            'classes': config.get('default_classes', ['person']),
        }]
    for cam in cameras:
