...
class FlowTracker:
    ...
    def __init__(self, cam_id: int, src: str, classes: list[str], cfg: dict, tasks: list[str] | None = None, mode: str = "both"):
            ...
            for tid in gone_ids:
                ...
                if self.tasks:
                    ...
                    for item, (st, conf, col) in results.items():
                        logger.info(f"PPE check {item} ID{tid}: {st} conf={conf:.2f}")
                ...
                for item, res in results.items():
                    ...
                    if st in self.alert_anomalies:
                        to_emails = self.email_cfg.get('ppe_to', '')
                        recipients = [a.strip() for a in to_emails.split(',') if a.strip()]
                        threading.Thread(
                            target=send_email,
                            args=(f'PPE Alert: {st}', f'Camera {self.cam_id} detected {st}', recipients, open(path, 'rb').read(), self.email_cfg),
                            daemon=True,
                        ).start()
