# Crowd Management System v65

This version provides person and PPE detection with optional duplicate frame dropping. The application is built with FastAPI and can ingest HTTP or RTSP camera streams. Results are stored in Redis and a simple PHP frontend displays reports.
...
python3 app.py
```
Then open `http://localhost:5002` in your browser. Use the **Cameras** page to add streams and **Settings** to adjust options. Tests can be executed with `pytest`:
```bash
