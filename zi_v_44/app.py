#!/usr/bin/env python3
"""Crowd management system version 44."""
...
async def helmet_report_page(request: Request, status: str = 'no_helmet'):
    return templates.TemplateResponse('helmet_report.html', {'request': request, 'cfg': config, 'status': status})


@app.get('/helmet_report_data')
async def helmet_report_data(start: str, end: str, status: str = 'no_helmet', min_conf: float | None = None, color: str | None = None):
    try:
        start_ts = int(datetime.fromisoformat(start).timestamp())
        end_ts = int(datetime.fromisoformat(end).timestamp())
    except Exception:
...
async def helmet_report_export(start: str, end: str, status: str = 'no_helmet', min_conf: float | None = None, color: str | None = None):
    data = await helmet_report_data(start, end, status, min_conf, color)
    if 'error' in data:
        return JSONResponse(data, status_code=400)
