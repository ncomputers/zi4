...
class FlowTracker:
    ...
    def __init__(self, cam_id: int, src: str, classes: list[str], cfg: dict, tasks: list[str] | None = None):
        ...
        nh_date_raw = self.redis.get('nohelmet_date')
        if nh_date_raw:
            nh_date = date.fromisoformat(nh_date_raw.decode())
        else:
            nh_date = self.prev_date
        if nh_date != date.today():
            self.redis.mset({'nohelmet_count': 0, 'nohelmet_date': date.today().isoformat()})
    ...
    def process_loop(self):
        ...
        while self.running or not self.frame_queue.empty():
            ...
            if date.today() != self.prev_date:
                ...
                self.redis.mset({'nohelmet_count': 0, 'nohelmet_date': self.prev_date.isoformat()})
            ...
            for tid in gone_ids:
                ...
                if 'helmet' in self.tasks:
                    for im in images:
                        st, conf, col = self.check_helmet(im)
                        votes.append((st, conf, col))
                    if votes:
                        cnt_no = len([v for v in votes if v[0] == 'no_helmet'])
                        cnt_he = len([v for v in votes if v[0] == 'helmet'])
                        if cnt_no > cnt_he:
                            status = 'no_helmet'
                            best_conf = max((c for s,c,_ in votes if s=='no_helmet'), default=0.0)
                        elif cnt_he > 0:
                            status = 'helmet'
                            idx = max((i for i,v in enumerate(votes) if v[0]=='helmet'), key=lambda i:votes[i][1])
                            best_conf = votes[idx][1]
                            color = votes[idx][2]
                        logger.info(f"Helmet check ID{tid}: {status or 'unknown'} conf={best_conf:.2f} color={color}")
                ...
                if status and best_conf >= self.helmet_conf_thresh:
                    ...
                    if status == 'no_helmet':
                        self.redis.incr('nohelmet_count')
