#!/usr/bin/env python3
"""Crowd management system version 20."""
...
def init_db(path: str) -> sqlite3.Connection:
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS counts(ts INTEGER PRIMARY KEY, in_count INTEGER, out_count INTEGER)"
    )
    return conn
...
def log_counts():
    ...
    with db_lock:
        db_conn.execute(
            "INSERT INTO counts(ts,in_count,out_count) VALUES (?,?,?)",
            (ts, in_c, out_c),
        )
        db_conn.commit()
...
async def report_data(start: str, end: str):
    ...
    with db_lock:
        rows = db_conn.execute(
            'SELECT ts,in_count,out_count FROM counts WHERE ts BETWEEN ? AND ? ORDER BY ts',
            (start_ts, end_ts),
        ).fetchall()
    times = [datetime.fromtimestamp(r[0]).strftime('%Y-%m-%d %H:%M') for r in rows]
    ins = [r[1] for r in rows]
    outs = [r[2] for r in rows]
    return {'times': times, 'ins': ins, 'outs': outs}
...
def main():
    ...
    global db_conn
    db_conn = init_db(temp_cfg.get('db_path', 'history.db'))
